 (function() {
   try {
     const apiInitializer = require("discourse/lib/api").apiInitializer;
     const registerIcon = require("discourse-common/lib/icon-library").registerIcon;
     ...
     apiInitializer("1.19.0", function(api) {
       ...
       async function ensureTopicForChannel(channelId, channelName) {
         ...
         const internalCategoryId = 242; // Zendesk Tickets category
       ...
       function getCurrentChatChannel() {
         try {
           return api.container.lookup("service:chat")?.activeChannel || null;
         } catch {
           return null;
         }
       }
           ...
           btn.addEventListener("click", async (e) => {
             ...
             try {
               ...
               try {
                 ...
                 console.error("Zendesk returned non-JSON:", text);
               ...
               if (data.zendesk_url) window.open(data.zendesk_url, "_blank");
             } catch (err) {
     ...
     console.error("Zendesk chat script failed:", err);
   }
 })();
